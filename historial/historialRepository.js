    ...
    function get(req, res, next) {
           if (req.params.historialId) {
                var parameters = [];
                    ...
                    var query = "select a.Detalle, eq.nInventario, es.Estado, eq.Remito_Factura, o.OdeCompra, o.Fecha, eq.Obvs, s.Detalle as Ultimo  from Equipos eq  inner join  Articulos a on eq.id_Art = a.id_Art  inner join Estado es on eq.id_Estado = es.id_Estado  inner join OrdendeCompra o on eq.id_OC = o.Id_OC inner join Sector s on eq.id_Sec = s.id_Sec where eq.serial = @serial"
